...
function findPackageRoot$2(startDir) {
  let dir = startDir;
  for (let i = 0; i < 5; i++) {
...
const PACKAGE_ROOT$2 = findPackageRoot$2(__dirname$3);
const CLI_TOOL_DISPLAY_NAMES = {
  codex: "Codex CLI",
  "gemini-cli": "Gemini CLI",
  opencode: "opencode"
};
const RETRYABLE_ERRORS = /* @__PURE__ */ new Set(["EPERM", "EBUSY", "EACCES", "EXDEV"]);
const RETRY_COUNT = 3;
...
async function buildInstructionsContent(cliTool, configuredMcpServers, _options) {
  const warnings = [];
  const templateDir = join(PACKAGE_ROOT$2, "templates", "instructions");
...
const DEFAULT_INSTRUCTIONS_PATHS = {
...
async function installInstructions(config, _options) {
  const result = {
    success: true,
    written: [],
    skipped: [],
  ...
  const tools = ["codex", "gemini-cli", "opencode"];
  for (const tool of tools) {
    ...
    try {
      const targetPath = expandHome(
        toolConfig.instructions_path?.trim() || DEFAULT_INSTRUCTIONS_PATHS[tool]
      );
      ...
      const header = `<!-- AUTO-GENERATED BY CCG v${version} - DO NOT EDIT -->
...
const CLI_TOOL_TO_BACKEND = {
  "codex": "codex",
  "gemini-cli": "gemini",
  "opencode": "opencode"
};
function buildExecFlags(cliTool, modelId) {
...
function injectConfigVariables(content, config) {
  ...
  const RUNTIME_ALLOWLIST = /* @__PURE__ */ new Set(["WORKDIR"]);
  const unresolvedMatch = processed.match(/\{\{([A-Z][A-Z0-9_]*)\}\}/g);
  if (unresolvedMatch) {
    const unresolved = unresolvedMatch.map((m) => m.slice(2, -2)).filter((name) => !RUNTIME_ALLOWLIST.has(name));
...
async function promptRoutingConfig(defaults) {
  ...
  const frontendToolChanged = defaults?.frontend?.cli_tool !== frontendTool;
  const frontendDefault = frontendToolChanged ? CLI_TOOL_DEFAULTS[frontendTool].defaultModelId : defaults?.frontend?.model_id ?? CLI_TOOL_DEFAULTS[frontendTool].defaultModelId;
  ...
  const backendToolChanged = defaults?.backend?.cli_tool !== backendTool;
  const backendDefault = backendToolChanged ? CLI_TOOL_DEFAULTS[backendTool].defaultModelId : defaults?.backend?.model_id ?? CLI_TOOL_DEFAULTS[backendTool].defaultModelId;
  ...
  return {
    frontend: { cli_tool: frontendTool, model_id: frontendModelId },
    backend: { cli_tool: backendTool, model_id: backendModelId }
  };
}
...
async function init(options = {}) {
  ...
  let customRouting = null;
  if (!options.skipPrompt) {
    console.log();
    console.log(ansis.cyan.bold(`  \u{1F527} CLI \u5DE5\u5177\u914D\u7F6E`));
    console.log();
...
function getConfigAbsPath(tool) {
...
function convertMcpConfig(baseConfig, _targetTool) {
  ...
  if (isWindows() && typeof result.command === "string") {
    ...
    if (needsWrapping.includes(result.command)) {
      const originalArgs = result.args || [];
...
async function backupExternalConfig(tool) {
...
async function readExternalConfig(tool) {
  const configPath = getConfigAbsPath(tool);
...
async function writeExternalConfig(tool, data) {
  const configPath = getConfigAbsPath(tool);
  const { format } = CONFIG_PATHS[tool];
  await fs.ensureDir(join(configPath, ".."));
  const content = format === "toml" ? stringify(data) : JSON.stringify(data, null, 2);
  const tmpPath = `${configPath}.tmp`;
  await fs.writeFile(tmpPath, content, "utf-8");
  await fs.rename(tmpPath, configPath);
}
async function writeExternalMcp(tool, serverName, config) {
  const configPath = getConfigAbsPath(tool);
  ...
  try {
    ...
    const existingServers = getNestedValue(data, mcpKey) || {};
...
async function syncMcpToTool(tool, servers) {
  const results = [];
  ...
  while (true) {
    ...
    const { action } = await inquirer.prompt([{
      ...
      choices: [
        ...
        { name: `${ansis.cyan("\u{1F527}")} \u5DE5\u5177\u94FE\u914D\u7F6E`, value: "toolchain" },
...
async function syncExternalMcp() {
  console.log();
  ...
  const { targetTools } = await inquirer.prompt([{
    ...
    validate: (input) => input.length > 0 || "\u8BF7\u81F3\u5C11\u9009\u62E9\u4E00\u4E2A\u5DE5\u5177"
  }]);
  const { selectedServers } = await inquirer.prompt([{
    ...
    validate: (input) => input.length > 0 || "\u8BF7\u81F3\u5C11\u9009\u62E9\u4E00\u4E2A\u670D\u52A1"
  }]);
  const serversToSync = {};
    ...
    for (const tool of targetTools) {
      ...
      if (succeededServers.length > 0) {
        config.cli_tools_mcp[toolKey].servers = [
          .../* @__PURE__ */ new Set([...config.cli_tools_mcp[toolKey].servers, ...succeededServers])
        ];
...
export { update as A, writeCcgConfig as B, CLI_TOOL_DISPLAY_NAMES as C, diagnoseMcpConfig as D, isWindows as E, readClaudeCodeConfig as F, fixWindowsMcpConfig as G, writeClaudeCodeConfig as H, configMcp as I, version as J, checkForUpdates as a, buildInstructionsContent as b, changeLanguage as c, compareVersions as d, createDefaultConfig as e, createDefaultRouting as f, getCcgDir as g, getConfigPath as h, getCurrentVersion as i, getLatestVersion as j, getWorkflowById as k, getWorkflowConfigs as l, i18n as m, init as n, initI18n as o, installAceTool as p, installAceToolRs as q, installInstructions as r, installWorkflows as s, migrateConfig as t, migrateToV1_4_0 as u, needsMigration as v, readCcgConfig as w, showMainMenu as x, uninstallAceTool as y, uninstallWorkflows as z };
